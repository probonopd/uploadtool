...
if [ "$TRAVIS_COMMIT" != "$tag_sha" ] ; then
  ...
  if [ ! -z "$TRAVIS_JOB_ID" ] ; then
    read -r -d '' BODY <<EOF
Release generated by Travis CI build no. $TRAVIS_BUILD_NUMBER.

Build log: https://travis-ci.org/$REPO_SLUG/builds/$TRAVIS_BUILD_ID/

_Generated by [uploadtool](https://github.com/probonopd/uploadtool)._
EOF
  else
    BODY=""
  fi
